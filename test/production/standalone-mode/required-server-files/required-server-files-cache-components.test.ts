...
describe('required server files app router', () => {
  ...
  const setupNext = async ({
    ...
    next = await createNext({
      files: {
        app: new FileRef(join(__dirname, 'app-cache-components')),
    ...
    const rewriteMeta = await next.readJSON(
      '.next/server/app/rewrite/first-cookie.meta'
    )
    rewritePostpone = rewriteMeta.postponed

    expect(typeof rewritePostpone).toBe('string')
    expect(rewritePostpone.length).toBeGreaterThan(0)
  ...
  it('should not send cache tags in minimal mode for SSR', async () => {
    for (const path of ['/api/ssr/first', '/api/ssr/second']) {
      const res = await fetchViaHTTP(appPort, path, undefined, {
        redirect: 'manual',
      })
      expect(res.status).toBe(200)
